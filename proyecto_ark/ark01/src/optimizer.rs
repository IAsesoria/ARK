...
impl Optimizer {
    ...
    fn lr_now(&self) -> f32 {
        if (self.step as usize) < self.cfg.warmup_steps {
            ...
            let total_decay = 800_000.0f32; // Modificado para un entrenamiento largo y estable de 800k pasos
    ...
    fn clip_all_grads(&self, weights: &mut ModelWeights) {
        ...
        // FIX CRÍTICO: Acumular el gradiente de la capa de normalización final
        sum_sq += weights.gamma_final_grad.data.iter().map(|x| x * x).sum::<f32>();
        ...
        if norm > self.cfg.grad_clip && norm > 0.0 {
            ...
            // FIX CRÍTICO: Escalar el gradiente de gamma_final para preservar la dirección global
            ffi::scale_tensor_vdsp(weights.gamma_final_grad.data.as_mut_slice(), scale);
    ...
    pub fn step_all(&mut self, weights: &mut ModelWeights) {
        ...
        // 1. Clip global de gradientes (Desactivado doble clip por tensor para preservar dirección física)
    ...
    pub fn restaurar_momentos(
        ...
        // Validar tamaño general (Permite de forma retrocompatible 271 o 272 tensores)
           ...
           (momentos_v.len() != esperados && momentos_v.len() != esperados + 1) {
            println!(
                "[optimizer] momentos Adam ignorados — \
                 tamaño incompatible ({} tensores, esperaba {} o {}). \
                 Optimizer partirá desde cero.",
                momentos_m.len(), esperados, esperados + 1
            );
            return;
        }
        ...
        // Restaurar momentos de Gamma Final (se encuentran en el índice 271)
        if momentos_m.len() == esperados + 1 {
            let idx = esperados;
            if momentos_m[idx].len() == weights.gamma_final_m.data.len() {
                weights.gamma_final_m.data.copy_from_slice(&momentos_m[idx]);
                weights.gamma_final_v.data.copy_from_slice(&momentos_v[idx]);
            }
        }
