// src/main.rs — ARK v1.3 "METAL-REASONER"
...
fn print_banner() {
    println!("╔");
    println!("║  ARK v1.3 — ZERO-COPY METAL-REASONER ENGINE");
    ...
    println!("  Tokenizador: BPE 32k+ v7 — {} tokens (SentencePiece)", 32_308);
    println!();
}
...
fn parse_args(args: &[String], cfg: &mut Config) {
    // Modo ayuda
    ...
    for arg in args {
        let arg: &str = arg; // Coacción explícita a &str que resuelve toda la inferencia de tipos
        if !arg.starts_with("--") {
            continue;
        }
        let arg = &arg[2..];
        ...
        match key.as_str() {
            ...
            "loss_scale_max" | "scale_max" => cfg.loss_scale_max = parse_f32(val, "loss-scale-max"),
...
fn main() -> anyhow::Result<()> {
    ...
    // Iniciar entrenamiento con ARK v1.3
