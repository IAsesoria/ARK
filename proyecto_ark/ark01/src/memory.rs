...
impl ModelWeights {
    ...
    pub fn all_grad_slices_mut(&mut self) -> impl Iterator<Item = &mut f32> {
        ...
        // FIX CRÍTICO: Incluir los gradientes de gamma_final en el chequeo de NaNs del entrenador
        let gamma = self.gamma_final_grad.data.as_mut_slice().iter_mut();
        ...
        embed.chain(gamma).chain(layers)
    }
}
