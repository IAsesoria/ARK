...
.L_rmnbwd_update4:
    ...
    // dx[i] = gamma * inv_rms * (dy - x_norm*(dot/dim))
    // Usamos v5 para no destruir los datos en v0 y v1
    fmul v5.4s, v1.4s, v16.4s   // v5 = x_norm * (dot/dim)
    fsub v5.4s, v0.4s, v5.4s    // v5 = dy - x_norm * (dot/dim)

    fmul v3.4s, v2.4s, v17.4s   // v3 = gamma * inv_rms
    fmul v3.4s, v3.4s, v5.4s    // v3 = dx
    str  q3, [x3], #16

    // dgamma[i] += dy[i] * x_norm[i] (Usa v0 y v1 directamente de los registros)
    ldr  q4, [x6]
...
.L_rmnbwd_updatetail:
    ...
    // dx[i] = gamma * inv_rms * (dy - x_norm*(dot/dim))
    // Usamos s4 para preservar los valores de s0 y s1 intactos
    fmul s4, s1, s16        // s4 = x_norm * (dot/dim)
    fsub s4, s0, s4         // s4 = dy - x_norm * (dot/dim)

    fmul s3, s2, s8         // s3 = gamma * inv_rms
    fmul s3, s3, s4         // s3 = dx
    str  s3, [x3], #4

    // dgamma[i] += dy[i] * x_norm[i] (Usa s0 y s1 de los registros)
    ldr  s3, [x6]
    fmadd s3, s0, s1, s3
    str  s3, [x6], #4
